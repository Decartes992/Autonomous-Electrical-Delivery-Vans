...
void ask_for_current_location(aedvs* head, buildings* buildingz, int Numberofbuildings)
{
	...
	for (i = 1; i <= NumeroAEDV; i++) {
		...
		printf("For buildingside: Enter 1 for NW, 2 for N, 3 for NE, 4 for W, 5 for E, 7 for SW, 8 for S, 9 for SE \n");
	...
	while (curr_parked) {
		...
		if (ask_for_whether_to_start_aedv == ZERO) {
			...
			CUP(Indent_location, Text_Position_Y + cursoroffset);
				...
				printf("For buildingside: Enter 1 for NW, 2 for N, 3 for NE, 4 for W, 6 for E, 7 for SW, 8 for S, 9 for SE");
				printf(CSI "?25h");
			CUP(Indent_location, Text_Position_Y + cursoroffset);
...
void set_location(aedvs* curr_parked) {
	...
	curr_parked->current.intersection_location.X = curr_parked->current.gridl.X;
	curr_parked->current.intersection_location.Y = curr_parked->current.gridl.Y;
	...
	curr_parked->destination.intersection_location.X = curr_parked->destination.gridl.X;
	curr_parked->destination.intersection_location.Y = curr_parked->destination.gridl.Y;
	/*Calculate distance*/
...
void print_current_destination(aedvs* tomove, aedvs* tomovellhead) {
	if (tomovellhead) {
		...
		CUP(tomove->current.gridl.X, tomove->current.gridl.Y);                       // postition cursor
