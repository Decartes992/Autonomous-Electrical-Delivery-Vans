...
void read_activevehicle(aedvs** aedv_) {
	...
	activevehicle = fopen("Activeaedv.txt", "r+");
...
void read_ordersfile(aedvs** aedv_) {
	...
	orders = fopen("Events.txt", "r+");

	int time;

	while (fscanf(orders, "%d %c %d %d %d\n", &order.time, &order.event, &order.origin, &order.destination, &order.weight) != EOF)
	{
		sender = *getCustomerInfo(order.origin);
		recipient = *getCustomerInfo(order.destination);
		curr.source.buildingl = getBuildingNumber(sender.building);
		curr.destination.buildingl = getBuildingNumber(recipient.building);
		curr.source.buildingside = stringToEnum(sender.entrance);
		curr.destination.buildingside = stringToEnum(recipient.entrance);
		curr.ordertime = order.time;
		curr.senderID = sender.custNum;
		curr.receiverID = recipient.custNum;
		//printf("%d %d\n", curr.source.buildingl, curr.destination.buildingl);



		set_location(&curr);
		//printf("%d %d %d %d %d\n", curr.source.buildingl, curr.source.buildingside, curr.destination.buildingl, curr.destination.buildingside, curr.ordertime);
		add_to_aedv_list(aedv_, curr);
	}
	fclose(orders);
	getchar();
			...
			if (rec_no > 0)
			{
				...
				*/
				...
				if (aedv_rec.aedvrec.status == ACTIVE)
				{
					...
					aedv_rec.aedvrec.current.intersection_location.X = aedv_rec.aedvrec.current.gridl.X;
					aedv_rec.aedvrec.current.intersection_location.Y = aedv_rec.aedvrec.current.gridl.Y;
...
Customer* getCustomerInfo(int customerID) {
	...
	while (fscanf(customerfile, "%d %s %s %s %s %d\n",
		&customer->custNum, customer->firstName, customer->lastName,
		customer->building, customer->entrance, &customer->floor) != EOF) {


		/*printf("%d %s %s %s %s %d\n",
			customer->custNum, customer->firstName, customer->lastName,
			customer->building, customer->entrance, customer->floor);*/

		if (customer->custNum == customerID) {
			return customer;
		}
	}
...
int getBuildingNumber(char arr[MAXBUILDINGNAME]) {
	int asciishift = 65;    // ASCII VALUE OF 'A' IS 65
	int street = arr[0] - asciishift;
	int avenue = arr[1] - asciishift;
	return street * 10 + avenue;
}
enum buildingsides stringToEnum(const char* side) {
	if (strcmp(side, "NW") == 0) return NW;
	else if (strcmp(side, "N") == 0) return NW;
	else if (strcmp(side, "NE") == 0) return NW;
	else if (strcmp(side, "W") == 0) return NW;
	else if (strcmp(side, "E") == 0) return SE;
	else if (strcmp(side, "SW") == 0) return SW;
	else if (strcmp(side, "S") == 0) return SW;
	else if (strcmp(side, "SE") == 0) return SW;
	else return NW; // Invalid input
}

void deliveryfile (aedvs aedv_) {
	_set_fmode(_O_TEXT);

	FILE* delivery = fopen("Delivery.txt", "a");
	fprintf(orders, "%d %c %d %d %d\n", deliverynumber, 'D', aedv_.senderID, aedv_.receiverID, 0);
	deliverynumber++;
